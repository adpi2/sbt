...
object ScopeFilter {
  ...
  def in(scopes: Seq[Scope]): ScopeFilter = {
    ...
    new ScopeFilter {
      def apply(data: Data): Set[Scope] =
        data.allScopes.set.intersect(scopeSet)
    }
  }
  ...
  private[sbt] val allScopes: Initialize[AllScopes] = Def.setting {
    ...
    val grouped: ScopeMap =
      scopes
        .groupBy(_.project)
        .map {
          case (k, v) => k -> v.groupBy(_.config).map { case (k, v) => k -> v.groupBy(_.task) }
        }
